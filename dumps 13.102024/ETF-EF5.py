...
class PortfolioOptimizer:
    def __init__(self, etf_metrics, risk_free_rate=0.02):
        ...
        self.results_matrix, self.optimal_weights, self.optimal_metrics, self.all_weights = self.optimize_portfolio(10000)

    def generate_random_weights(self, num_assets):
        while True:
            weights = np.random.choice(np.arange(0, 1 + self.weight_step_size, self.weight_step_size), size=num_assets)
    ...
    def optimize_portfolio(self, num_portfolios):
        results_matrix = np.zeros((4, num_portfolios))  # Adding an additional row to store weights
        all_weights = []
...
class EfficientFrontierPlotter:
    ...
    def plot_efficient_frontier(self, save_path='efficient_frontier.png'):
        ...
        scatter = ax.scatter(self.results[1], self.results[0], c=self.results[2], cmap='viridis', marker='o', edgecolor='k', alpha=0.7)
        plt.colorbar(scatter, label='Sharpe Ratio')
        ax.set_title('Efficient Frontier')
        ax.set_xlabel('Volatility (Standard Deviation)')
        ax.set_ylabel('Expected Return')
        ax.scatter(optimal_risk, optimal_return, color='red', marker='*', s=200, label='Optimal Risk Adjusted Portfolio')
        ax.legend()
        ax.text(0.02, 0.95, 'The Sharpe Ratio measures the risk-adjusted return of a portfolio.\n'
...
def main():
    etfs_list = [
        ...
        ['IWM', 'EFA', 'TLT', 'TIP', 'DBC'],
        ['QQQ', 'TLT', 'HYG', 'XLK', 'XLV'],
        ['IVV', 'SOXX', 'ARKK', 'XLC', 'XLF'],
        ['SCHD', 'SCHF', 'SCHP', 'SCHB', 'SCHA'],
        ['MTUM', 'USMV', 'VLUE', 'QUAL', 'SPYG'],
        ['FTEC', 'FENY', 'FSTA', 'FHLC', 'FMAT'],
        ['IGV', 'XBI', 'ITA', 'IYT', 'XME'],
        ['EEM', 'ACWI', 'SPDW', 'VEU', 'GNR']
    ]
