...
class ETFMetrics:
    ...
    def calculate_individual_metrics(self):
        ...
        metrics['CAGR'] = (1 + metrics['Total Return']) ** (1 / (len(self.daily_returns) / 252)) - 1
...
class ReportGenerator:
    ...
    def condition_metrics_for_percentage(self):
        self.etf_metrics.metrics['Total Return (%)'] = self.etf_metrics.metrics['Total Return'] * 100
        self.etf_metrics.metrics['Annualized Return (%)'] = self.etf_metrics.metrics['Annualized Return'] * 100
        self.etf_metrics.metrics = self.etf_metrics.metrics.drop(columns=['Total Return', 'Annualized Return'])
