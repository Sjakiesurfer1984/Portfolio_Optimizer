...
class PortfolioOptimizer:
    ...
    def generate_random_weights(self, num_symbols):
        while True:
            # Generate random values in steps of 1, within the allowed weight range (max_etf_weight * 100)
            random_values = np.random.randint(0, int(self.max_etf_weight * 100) + 1, size=num_symbols)
            ...
            # Ensure that weights sum exactly to 100
            adjustment = 100 - np.sum(weights)
            weights[0] += adjustment

            # Check that none of the weights exceed the max allowed weight constraint after adjustment
            if np.all(weights <= self.max_etf_weight * 100) and np.sum(weights) == 100:
...
class EfficientFrontierPlotter:
    def __init__(self, optimizer):
        ...
        # self.results[0] = self.results[0]*100
        pprint(f"self.results[0] = {self.results[0]}")
...
# Step 2: On line 270 symbols = asset_list[0], 0 indicates it will analyse row 0 of the list that you populated in step 1. Likewise, 1 means the programme will analyse the portfolio you defined on line 1, etc. 
# Step 3: OPTIONAL: line 83 : self.max_etf_weight = 0.3  # Maximum weight for a single ETF. 0.3 corresponds to 30%, meaning a single ETF of the portfolio you defined in step 1, cannot exceed a maximum weight of 30%.
# and line 84: self.weight_step_size = 1  # Weight step size in increments of 1%. The programme calculated returns and risk for every possible combination of portfolio compositions in steps of 1%. ALterantively, you can set it to larger steps. 
